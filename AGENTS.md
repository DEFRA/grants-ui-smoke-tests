...
test/
  utils/
    ...
    backend.js                 # clearApplicationData() — DELETE /admin/test-data before each test
...
- `npm test`: run CDP-oriented smoke tests (requires `ENVIRONMENT` env var).
- `npm run test:local`: run smoke tests against local services.
- `npm run report:publish`: publish the Playwright HTML report to S3 via `RESULTS_OUTPUT_S3_PATH`.
...
**Password:** the stub accepts any value; the suite always sends `x`.
...
Before each test, `clearApplicationData()` calls `DELETE /admin/test-data` on `grants-ui-backend` to wipe any prior state, submissions, and locks for the test SBI/grant in one call. This ensures a clean journey start regardless of previous runs.
